	...
	while(ros::ok()){
		...
		if(odom_callback_flag && pc_callback_flag){
			...
			for(auto& pt : otsu_binary_pc_->points){
				...
				tmp_rgba_pt->points[0].a = 255.0;
				if(pt.intensity == 1.0){
					tmp_rgba_pt->points[0].g = 255.0;
					tmp_rgba_pt->points[0].b = 0.0;
					tmp_rgba_pt->points[0].r = 0.0;
				}else{
					tmp_rgba_pt->points[0].b = 255.0;
					tmp_rgba_pt->points[0].g = 255.0;
					tmp_rgba_pt->points[0].r = 255.0;
					/* tmp_rgba_pt->points[0].a = 100.0; */
					tmp_rgba_pt->points[0].a = 0.0;
				}
				*rgba_pc += *tmp_rgba_pt;
			}
...
pcl::PointCloud<pcl::PointXYZINormal>::Ptr IntensityPartition::otsu_pc_generator(void)
{
	...
	pass.setFilterLimits(-0.5, intensity_max_all);
