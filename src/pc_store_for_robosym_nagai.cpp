...
private:
	...
	CloudRGBAPtr input_pc_ {new CloudRGBA};
	CloudINormalPtr polar_pc_ {new CloudINormal};
	CloudRGBAPtr store_pc {new CloudRGBA};
	struct GA{
...
IntensityPartition::IntensityPartition(void)
{
	...
	/* sub_pc = n.subscribe("/recognition/road_recognizer/road_cloud_publisher/cloud/intensity", 10, &IntensityPartition::fresh_pc_callback, this); */
	sub_pc = n.subscribe("/robosym_pc", 10, &IntensityPartition::fresh_pc_callback, this);
	sub_odom = n.subscribe("/odom/complement", 10, &IntensityPartition::odom_callback, this);
	advertise_pc = n.advertise<sensor_msgs::PointCloud2>("/robosym_pc_store", 10);
}
...
void IntensityPartition::execution(void)
{
	...
	while(ros::ok()){
		...
		if(odom_callback_flag && pc_callback_flag){
			...
			CloudRGBAPtr otsu_binary_pc_ {new CloudRGBA};

			otsu_binary_pc_ = input_pc_;
			...
			std::cout << "input_pc size : " << input_pc_->points.size() << std::endl;
			std::cout << "rgba_pc size : " << rgba_pc->points.size() << std::endl;
